...
int LogosExecutionZoneWalletModule::create_new(
    ...
    FfiCreateWalletOutput out = wallet_ffi_create_new(config_utf8.constData(), storage_utf8.constData(), password_utf8.constData());
    walletHandle = out.wallet;
    if (out.mnemonic) {
        wallet_ffi_free_string(out.mnemonic);
    }
    if (!walletHandle) {
...
QString LogosExecutionZoneWalletModule::send_public_transaction(const QString& tx_json) {
    ...
    // program_id (32-byte hex) -> FfiProgramId (uint32[8], raw 32 bytes, like upstream)
    ...
    FfiProgramId fpid{};
    memcpy(fpid.data, programId.data, 32);

    // signer drives the per-account signing flag for resolve_public_account
    FfiBytes32 signer{};
    ...
    // Resolve each account into an FfiAccountIdentity (sign iff it is the signer).
    const QJsonArray accountsArr = obj["accounts"].toArray();
    std::vector<FfiAccountIdentity> identities;
    identities.reserve(accountsArr.size());
    auto freeIdentities = [&identities]() {
        for (FfiAccountIdentity& a : identities) wallet_ffi_free_account_identity(&a);
    };
    for (int i = 0; i < accountsArr.size(); ++i) {
        FfiBytes32 id{};
        if (!hexToBytes32(accountsArr[i].toString(), &id)) {
        ...
        const bool needsSign = (memcmp(id.data, signer.data, 32) == 0);
        FfiAccountIdentity acc{};
        const WalletFfiError rerr = wallet_ffi_resolve_public_account(id, needsSign, &acc);
        if (rerr != SUCCESS) {
            qWarning() << "send_public_transaction: resolve_public_account error" << rerr << "at" << i;
            freeIdentities();
            return transferResultToJson(nullptr, QStringLiteral("resolve_public_account error ") + QString::number(rerr));
        }
        identities.push_back(acc);
    }

    // instruction hex bytes (Vec<u32> serialized LE) -> uint32 words
    QByteArray instrBytes;
    if (!hexToBytes(obj["instruction"].toString(), instrBytes) || (instrBytes.size() % 4) != 0) {
    ...
    std::vector<uint32_t> instrWords(static_cast<size_t>(instrBytes.size()) / 4);
    for (size_t k = 0; k < instrWords.size(); ++k) {
        const uint8_t* p = reinterpret_cast<const uint8_t*>(instrBytes.constData()) + k * 4;
        instrWords[k] = static_cast<uint32_t>(p[0]) | (static_cast<uint32_t>(p[1]) << 8)
                      | (static_cast<uint32_t>(p[2]) << 16) | (static_cast<uint32_t>(p[3]) << 24);
    }

    // rc6 NATIVE program-id-based generic public transaction (replaces our
    // forked wallet_ffi_send_public_transaction; the wallet-ffi fork is dropped).
    FfiTransactionResult result{};
    const WalletFfiError error = wallet_ffi_send_generic_public_transaction(
        walletHandle,
        identities.data(),
        static_cast<uintptr_t>(identities.size()),
        instrWords.data(),
        static_cast<uintptr_t>(instrWords.size()),
        fpid,
        &result);
    freeIdentities();
    ...
    r["tx_hash"] = result.tx_hash ? QString::fromUtf8(result.tx_hash) : QString();
    const QString resultJson = QJsonDocument(r).toJson(QJsonDocument::Compact);
