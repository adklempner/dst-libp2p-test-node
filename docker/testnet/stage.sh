...
# canonical copy there. dst-libp2p vendors logos-lez-rln pieces (see docker/vendor/)
# rather than submoduling it, and the image build must be self-contained, so the
# stage script lives in the build context. jq is present in the image + on host.
